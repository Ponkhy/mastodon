...
class LoginForm extends React.PureComponent {
  ...
  handleChange = ({ target }) => {
    this.setState({ value: target.value, isLoading: true }, () => this._loadOptions());
  };
  ...
  _loadOptions = throttle(() => {
    ...
    api().get('/api/v1/peers/search', { params: { q: value } }).then(({ data }) => {
      if (value.includes('.') && !data.includes(value)) {
        data.unshift(value);
      }
            ...
            <div className='search__popout__menu'>
              {options.map((domain, i) => (
                <button key={domain} onMouseDown={this.handleOptionClick} data-index={i} className={classNames('search__popout__menu__item', { selected: selectedOption === i })}>
                  {domain.split(new RegExp(`(${this.state.value})`, 'gi')).map((part, i) => (
                    part.toLowerCase() === this.state.value.toLowerCase() ? (
                      <mark key={i}>
                        {part}
                      </mark>
                    ) : (
